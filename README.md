...
- 🧠 **LangGraph reasoning agent** with chain-of-thought audit trail
- 📈 **Buying intent scoring** that fuses news, hiring, tech stack, seniority, and **buyer engagement** (the strongest signal there is)
- 📬 **Buyer-side mail timeline** — opens, clicks, replies (with sentiment + intent classification) on every lead
- ✉️ **Per-lead personalised emails** generated by LLM at runtime, never template substitution
...
- ⚙️ **Compliance built-in** — opt-out hard blocks, spam scoring, CAN-SPAM unsubscribe footer
...
| # | Where | What to see |
|---|---|---|
| 1 | **Dashboard** | Pipeline of seeded leads with the agent's plain-English reason on every card. Live activity feed on the right. |
| 2 | **Click any lead** | Full lead profile, **Buying Intent** card with explainable reasons, **Prior Engagement** timeline (buyer side), and the chain of thought. |
| 3 | **Click "Run Agent Reasoning"** | A fresh decision is generated live by the LLM, complete with signal analysis, options considered, and confidence. |
| 4 | **Click "Simulate buyer reply"** on the engagement panel | The inbound webhook classifies the reply, persists it, **bumps the buying intent score**, and triggers reasoning. Watch the score and timeline update. |
| 5 | **Sequences → Preview emails** | Pick a lead and the agent generates the 3 personalised emails (intro / follow-up / breakup) using that lead's enrichment context. A/B variant + spam score per email. |
| 6 | **Navbar search** | Type a lead name, company, title, or email. Debounced live results with keyboard navigation. |
| 7 | **Notifications bell** | Real-time agent activity, type-aware icons, unread badge, mark-all-read. |
| 8 | **Analytics → Reply Rate Trend** | Rising trend over 4 weeks. The self-improving loop, made visible. |
| 9 | **Agent Feed** | Full audit log. Every decision the agent ever made, filterable by type and confidence. |
| 10 | **Import → upload `backend/demo-leads.csv`** | 15 real B2B companies (Clay, Linear, Vercel, Mercury, Pinecone, etc.) flowing through enrichment in real time. |
...
### Buying intent (explainable scoring)

Buying intent is computed in `backend/app/enrichment/intent_enricher.py` and surfaced on every lead with a `reasons` list. Every point in the score has a sentence behind it.

| Bucket | Cap | Driven by |
|---|---|---|
| Recent funding / IPO | 30 | News headlines, recency-weighted (≤90 days = full points) |
| Hiring activity | 25 | Number of hiring mentions in the news |
| Tech replacement | 30 | Competitor tooling already in their stack (Outreach, Salesloft, Apollo, etc.) |
...
The score is recomputed automatically every time the buyer engages. SendGrid webhooks (open/click/bounce) and inbound replies both trigger an intent refresh, so the dashboard reflects what the buyer is actually doing within seconds.

### Buyer-side mail timeline

Every send, open, click, reply, bounce, and unsubscribe is captured in the `email_events` table and rendered in the **Prior Engagement** panel on the lead detail page. Replies are LLM-classified into sentiment + intent (`interested`, `not_now`, `meeting_requested`, `unsubscribe`, etc.) and feed back into reasoning for the next decision.

For demo and testing, a **Simulate buyer reply** button POSTs to `/api/webhooks/reply` with whatever text you type — useful for showing the agent reacting to a real reply without configuring an inbound mailbox.
...
│   │   │
│   │   ├── api/                         # FastAPI routes
│   │   │   ├── leads.py                 # CRUD + CSV import + /events timeline
│   │   │   ├── agent.py
│   │   │   ├── sequences.py             # List + create + per-lead email preview
│   │   │   ├── webhooks.py              # SendGrid + inbound reply (refreshes intent)
│   │   │   ├── analytics.py
│   │   │   ├── settings.py              # /api/settings/status (no secrets)
│   │   │   └── auth.py
│   │   │
...
│   │   ├── leads/                       # LeadTable, LeadEnrichmentView, LeadEngagementPanel, CSVUploader, StateTimeline
│   │   ├── analytics/                   # All charts (Recharts)
│   │   └── shared/                      # Sidebar, Navbar (live search + notifications), StatusBadge
│   │
│   ├── lib/
...
GET    /api/leads                  # List with state/search filters + pagination
                                   # Search matches name, email, title, and company name
...
### Settings
```http
GET    /api/settings/status        # Which integrations are configured (booleans only — no secrets)
```
...
3. ✅ **An analytics chart** showing reply rate climbing over 4 weeks
4. ✅ **One lead detail page** with the complete chain of thought, the buyer-side engagement timeline, and an explainable buying intent score
5. ✅ **A simulated buyer reply** that updates state, refreshes the intent score, and triggers fresh reasoning live
6. ✅ **Sequence email preview** generating three different personalised emails for the same lead

If those six things are visible, the project has shipped its core promise.
